...
const Dashboard = () => {
  ...
  const { data: overdueCount } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("invoices").select("id, invoice_date, payment_status, const { data } = await supabase.from("invoices").select("id, invoice_date, payment_status, contacts!invoices_contact_id_fkey(payment_terms)").in("payment_status", ["credit", "partial", "pending"]);;
      if (!data) return 0;
      const now = new Date();
