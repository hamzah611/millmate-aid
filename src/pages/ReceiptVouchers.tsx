...
const ReceiptVouchers = () => {
  ...
  const deleteTransferMutation = useMutation({
    mutationFn: async (record: any) => {
      ...
      const pairSuffix = vn.endsWith("-B") ? "-A" : "-B";
  ...
  // Build unified list: regular receipts + transfer-B records
  const unifiedRows = useMemo(() => {
    if (!vouchers) return [];
    
    return vouchers.map(v => {
      const vn = v.voucher_number || "";
      const isTransferB = vn.endsWith("-B");
  ...
  // For transfer rows, we need to find the -A pair to get "from" info
  const { data: transferPairs } = useQuery({
    queryKey: ["receipt-transfer-pairs", unifiedRows.filter(r => r._rowType === "transfer").map(r => r.voucher_number)],
    queryFn: async () => {
      ...
      for (const tr of transferRows) {
        const baseNum = (tr.voucher_number || "").replace(/-B$/, "");
        const { data: aRecords } = await supabase
          .from("payments")
          .select("*")
          .eq("voucher_number", baseNum + "-A")
          .limit(1);
        if (aRecords?.[0]) {
          pairMap.set(tr.id, aRecords[0]);
        }
      }
      return pairMap;
    },
          ...
          <TableBody>
            {isLoading ? (
              ...
              unifiedRows.map((v) => {
                ...
                const pairA = isTransfer ? transferPairs?.get(v.id) : null;
                const baseNum = isTransfer ? (v.voucher_number || "").replace(/-B$/, "") : null;
                const fromName = pairA ? getAccountName(pairA) : "—";
                const toName = isTransfer ? getAccountName(v) : null;
                  ...
                  <TableRow key={v.id}>
                    ...
                    <TableCell>
                      <Badge variant={isTransfer ? "secondary" : "outline"} className="text-xs">
                        {isTransfer ? t("voucher.transferIn") : t("voucher.receipt")}
                      </Badge>
                    </TableCell>
